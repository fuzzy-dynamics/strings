...
Both `install-claude.sh` and `install-codex.sh` require the SSH ControlMaster to be alive (run `reconnect-ssh.sh` first if it isn't). They run `npm install -g <package>` on the remote, smoke-test with `<provider> --version`, and update `services.providers.<provider>` in the index. A failed install sets `services.providers.<provider>.installed=false` — the base machine itself is unaffected and stays selectable for `gecko` runs.
...
| Multiple machines share a port on the laptop | Expected — Electron allocates dynamic local ports per machine; only one is selected at a time in the renderer. | The selected machine is held in renderer memory only; ask the user which one they have selected. |
