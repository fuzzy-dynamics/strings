#!/usr/bin/env bash
# install-claude.sh <name>
#
# Installs Claude Code CLI on a remote machine. Per machine-provisioning-spec.md
# §4.3:
#   - Requires state="ready" (base machine working).
#   - Failed provider install does NOT demote machine to broken; sets
#     lastProviderError.claude and services.providers.claude.installed=false.
...
NAME="${1:-}"
if [[ -z "$NAME" ]]; then
  printf '{"ok":false,"stage":"parse-args","message":"usage: install-claude.sh <name>"}\n'
  exit 2
fi
if [[ "$NAME" == "local" ]]; then
...
emit_progress info "download" "fetching Claude Code installer"

# Claude Code CLI is distributed as an npm package. The installer here is
# really `npm install -g @anthropic-ai/claude-code` run on the remote — no
...
if ! install_log=$(with_timeout 90 "install" -- \
  ssh_run "$NAME" -q -- "npm install -g @anthropic-ai/claude-code 2>&1"); then
...
version=""
if ! version=$(with_timeout 15 "smoke" -- ssh_run "$NAME" -q -- "claude --version 2>&1"); then
...
jq -nc \
  --arg name "$NAME" \
  --arg version "$version" \
  '{ok:true,name:$name,stage:"done",provider:"claude",version:$version}'

emit_progress info "done" "claude installed: $version"
