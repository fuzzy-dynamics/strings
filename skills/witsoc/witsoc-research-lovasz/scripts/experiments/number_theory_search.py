...
def factor(n: int) -> dict[int, int]:
    if n < 1:
        raise ValueError(f"factor() requires a positive integer, got {n}")
    d: dict[int, int] = {}
    p = 2
    while p * p <= n:
        while n % p == 0:
...
def main() -> int:
    ...
    if args.mode in ("factor", "abundancy"):
        if args.n is None:
        ...
        if args.mode == "factor":
            ...
            result = {
                "n": args.n,
                "sigma_over_n": [r.numerator, r.denominator],
                "is_integer": r.denominator == 1,
            }
    else:
