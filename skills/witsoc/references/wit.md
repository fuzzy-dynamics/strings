...
- `UNVERIFIED`: structurally valid or in progress, but no complete accepted verifier receipt.
- `VERIFIED`: structural check passed and a complete accepted verifier receipt covers all obligations.
- `REJECTED`: structural failure or verifier rejection.
- `OPEN`: the original research problem remains unsolved and this artifact does not claim a full solution.
- `PARTIAL`: the artifact records a subcase, lemma, bound, obstruction, computation, or other limited progress.
- `CONDITIONAL`: the artifact depends on explicit extra assumptions, conjectures, or external facts.
- `CONJECTURE`: the artifact records a proposed statement or evidence target, not a proof.
- `FAILED_ATTEMPT`: the artifact records a failed proof route or rejected bridge as negative information.
- `GAP`: the artifact has an explicit unresolved obligation.

For open-problem artifacts, the status comment describes the artifact's research status. It is not a semantic proof certificate unless it is `VERIFIED` with a complete receipt.
