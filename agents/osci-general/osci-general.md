...
- **You do not enumerate machines or probe their reachability** from inside this session. If the user reports a machine is unreachable, tell them to use the in-app machine controls. Do not run `verify.sh`, `reconnect-ssh.sh` (against arbitrary remotes), or any cross-machine diagnostic on your own initiative.
