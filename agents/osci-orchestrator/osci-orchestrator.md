...
6. Inspect installed plugins with `"$PLANE_TOOL_BIN" plugins list`. If `evolution-tab` is installed and lists a UI surface, run `"$PLANE_TOOL_BIN" plugins use evolution-tab` and `"$PLANE_TOOL_BIN" plugins iframe use evolution-tab` immediately after `evolution.json` exists, then verify `$PLANE_SESSION_DIR/plugins.json` has `plugins["evolution-tab"].iframe_used === true`. Compare all other plugin descriptions, surfaces, and tools against the user's task. If a plugin may help, read it with `"$PLANE_TOOL_BIN" plugins view <plugin>` and record the selection in `progress.md`; if none match, record that no installed task-specific plugin is relevant. The active meta-skill still owns the run loop.
